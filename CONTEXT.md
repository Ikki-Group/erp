...
## Web data-fetching terms

Vocabulary for the `apps/web` data-fetching layer. Decisions (the reasoning) live in `docs/adr/0015`–`0016`.

| Term                         | Definition                                                                                                                                                                                                                                                                                                                                                                                 |
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Query-key factory**        | The single canonical builder (`createResourceKeys(feature, resource)`) of TanStack Query keys, producing a structured `[feature, resource, kind, params]` tuple. The only sanctioned way to make a key; prefix structure makes partial-match invalidation unambiguous.                                                                                                                     |
| **Location-scoped endpoint** | A query endpoint whose data belongs to the active location. Its key folds the active `locationId` in as a `{ loc }` segment, so each location's data caches separately (instant switch-back) and a location switch is a natural cache-miss — no blanket invalidation. The folded location is a cache concern; `locationId` is still sent as a server param where the contract requires it. |
| **Freshness tier**           | A named staleness level (`static` / `standard` / `volatile` / `realtime`) an endpoint opts into, instead of a raw `staleTime`. Maps to concrete `staleTime`/`gcTime`/`refetchInterval` in one place, making "how fresh must this be?" an explicit, reviewable choice.                                                                                                                      |
