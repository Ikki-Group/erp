...
export const MasterErrors = {
	...
	createFailed: () =>
		new InternalServerError('Material creation failed', {
			code: ERROR_CODES.MATERIAL_CREATE_FAILED,
		}),

	skuAlreadyExists: (sku: string) =>
		new ConflictError(`Material SKU "${sku}" already exists`, {
			code: ERROR_CODES.MATERIAL_SKU_ALREADY_EXISTS,
		}),

	nameAlreadyExists: (name: string) =>
		new ConflictError(`Material name "${name}" already exists`, {
...
export const CategoryErrors = {
	notFound: (id: number) =>
		new NotFoundError(`Material category with ID ${id} not found`, {
	...
	nameAlreadyExists: (name: string) =>
		new ConflictError(`Material category name "${name}" already exists`, {
...
export const UomErrors = {
	...
	createFailed: () =>
		new InternalServerError('UOM creation failed', { code: ERROR_CODES.UOM_CREATE_FAILED }),

	codeAlreadyExists: (code: string) =>
		new ConflictError(`UOM code "${code}" already exists`, {
			code: ERROR_CODES.UOM_CODE_ALREADY_EXISTS,
		}),
} as const
...
export const LocationErrors = {
	notFound: (id: number) =>
		new NotFoundError(`Material-Location assignment with ID ${id} not found`, {
			code: ERROR_CODES.MATERIAL_LOCATION_NOT_FOUND,
		}),

	notAssigned: (materialId: number, locationId: number) =>
		new NotFoundError(`Material ${materialId} is not assigned to location ${locationId}`, {
			code: ERROR_CODES.MATERIAL_NOT_ASSIGNED_TO_LOCATION,
		}),

	alreadyAssigned: (materialId: number, locationId: number) =>
		new ConflictError(`Material ${materialId} is already assigned to location ${locationId}`, {
			code: ERROR_CODES.MATERIAL_LOCATION_ALREADY_ASSIGNED,
		}),

	assignFailed: () =>
		new InternalServerError('Failed to assign material to location', {
...
export const ConversionErrors = {
	notFound: (id: number) =>
		new NotFoundError(`Material conversion with ID ${id} not found`, {
			code: ERROR_CODES.MATERIAL_CONVERSION_NOT_FOUND,
		}),

	createFailed: () =>
		new InternalServerError('Material conversion creation failed', {
			code: ERROR_CODES.MATERIAL_CONVERSION_CREATE_FAILED,
		}),

	invalidFactor: (factor: string) =>
		new BadRequestError(`Invalid conversion factor: ${factor}. Must be a positive number.`, {
			code: ERROR_CODES.MATERIAL_CONVERSION_INVALID_FACTOR,
		}),

	uomAlreadyExists: () =>
		new ConflictError('Material conversion for this UOM already exists', {
