    ...
    return cache.wrap(cacheKey, async () => {
      return record('AnalyticsService.getPnL', async () => {
        ...
        const glItems = await db
          ...
          .innerJoin(
            db.select({ id: sql`id`, date: sql`date` }).from(sql`journal_entries`).as('entries'),
            eq(journalItemsTable.journalEntryId, sql`entries.id`)
          )
          .where(
            and(
              gte(sql`entries.date`, startDate),
              lte(sql`entries.date`, endDate)
            )
          )
