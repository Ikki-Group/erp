...
export class HRService {
	...
	async handleClockIn(data: ClockInDto, actorId: number): Promise<AttendanceDto> {
		return record('HRService.handleClockIn', async () => {
			...
			if (existing) throw new ConflictError(`Employee with ID ${data.employeeId} is already clocked in`, 'ALREADY_CLOCKED_IN')
	...
	async handleClockOut(data: ClockOutDto, actorId: number): Promise<AttendanceDto> {
		return record('HRService.handleClockOut', async () => {
			...
			if (!attendance) throw new NotFoundError(`Attendance with ID ${data.id} not found`, 'ATTENDANCE_NOT_FOUND')

			if (!attendance.clockIn) throw new ConflictError(`Attendance with ID ${data.id} is not clocked in`, 'NOT_CLOCKED_IN')
			if (attendance.clockOut) throw new ConflictError(`Attendance with ID ${data.id} is already clocked out`, 'ALREADY_CLOCKED_OUT')
