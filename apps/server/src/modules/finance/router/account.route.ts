...
export function initAccountRoute(module: FinanceServiceModule) {
  ...
  return new Elysia({ prefix: '/account' })
    ...
    .get(
      ...
      { query: zRecordIdDto, response: createSuccessResponseSchema(dto.AccountDto), auth: true },
    )
    .post(
      '/create',
      ...
      { body: dto.AccountCreateDto, response: createSuccessResponseSchema(zRecordIdDto), auth: true },
    )
    .patch(
      '/update',
