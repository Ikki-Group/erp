    ...
    for (const item of items) {
      ...
      await record(`MovementLogic.handleStockOut.${type}.item:${materialId}`, async () => {
        ...
        if (assignment.currentQty < qty) {
          throw new BadRequestError(`Insufficient stock for material ${materialId}: available ${assignment.currentQty}, requested ${qty}`)
        }
        ...
        await this.mLocationSvc.updateCurrentStock(
          materialId,
          locationId,
          { currentQty: newQty, currentAvgCost: currentAvgCost, currentValue: newQty * currentAvgCost },
          actorId,
        )
      })
    }
