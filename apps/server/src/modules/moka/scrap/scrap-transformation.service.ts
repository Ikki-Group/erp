...
import { stampCreate, takeFirst, type DbClient } from '@/core/database'
...
import type { MokaCategoryRaw, MokaProductRaw, MokaSalesDetailRaw } from './scrap-raw.types'
			...
			for (const cat of categories) {
				...
				if (!existing) {
					await this.db
						...
						.values({ name: cat.name, ...stampCreate(actorId) })
				}
			}
		})
	}
