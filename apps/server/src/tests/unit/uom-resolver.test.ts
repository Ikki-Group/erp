...
describe('resolveConversion', () => {
	test('returns identity conversion without a path', () => {
		expect(resolveConversion(1, 1, Qty.of('2.5'), [])).toEqual({ result: Qty.of('2.5'), path: [] })
	})

	test('resolves a multi-hop conversion with decimal precision', () => {
		const result = resolveConversion(1, 3, Qty.of('2'), [
			conversion(1, 2, '1000'),
			conversion(2, 3, '1000'),
		])
		expect(result?.result.toNumeric()).toBe('2000000.000000')
		expect(result?.path).toHaveLength(2)
	})

	test('resolves inverse conversion by dividing the factor', () => {
		const result = resolveConversion(2, 1, Qty.of('2500'), [conversion(1, 2, '1000')])
		expect(result?.result.toNumeric()).toBe('2.500000')
	})

	test('returns null when no path exists', () => {
		expect(resolveConversion(1, 3, Qty.of('2'), [conversion(1, 2, '1000')])).toBeNull()
	})
})
