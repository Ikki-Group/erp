...
export const materials = pgTable(
	'materials',
	{
		...
		baseUomId: integer('base_uom_id').notNull().references(() => uoms.id, { onDelete: 'restrict' }),
		defaultPurchaseUomId: integer('default_purchase_uom_id').references(() => uoms.id, { onDelete: 'set null' }),
		defaultStockUomId: integer('default_stock_uom_id').references(() => uoms.id, { onDelete: 'set null' }),
		defaultRecipeUomId: integer('default_recipe_uom_id').references(() => uoms.id, { onDelete: 'set null' }),
	...
	(t) => [
		...
		check('materials_type_chk', sql`${t.type} IN ('raw', 'semi_finished')`),
