...
import { roles, userRoleAssignments, users } from '@/database/schema'
import { ConflictError, NotFoundError, UnauthorizedError } from '@/shared/errors/http.error'
...
export class IamService {
  ...
  async getUsers(query: IamDto.UserQuery) {
    ...
    const where = and(
      search
        ...
        : undefined,
      isActive === undefined ? undefined : eq(users.isActive, isActive)
    )
  ...
  async updateUser(id: string, data: IamDto.UserUpdate) {
    ...
    const updateValues: Partial<typeof userData> & { passwordHash?: string } = { ...userData }
    if (password) {
  ...
  async getRoles(query: IamDto.RoleQuery) {
    ...
    const [totalResult] = await db.select({ value: count() }).from(roles).where(where)
    const total = Number(totalResult?.value ?? 0)

    const data = await db.select().from(roles).where(where).limit(limit).offset(offset).orderBy(desc(roles.createdAt))
