  ...
  if (!context) {
    ...
    return (
      <Button
        variant="ghost"
        size="icon-sm"
        className={cn('size-7 cursor-move opacity-70 hover:bg-transparent hover:opacity-100', className)}
        disabled
      >
        <GripHorizontalIcon />
      </Button>
...
function DataGridTableDndRows<TData>({
  ...
  const modifiers = useMemo(() => {
    const restrictToTableContainer: Modifier = ({ transform, draggingNodeRect }) => {
      ...
      return { ...transform, x: Math.max(minX, Math.min(maxX, x)), y: Math.max(minY, Math.min(maxY, y)) }
    }
