...
export const LocationAssignMemberDialog = createCallable<LocationAssignMemberDialogProps>((props) => {
  ...
  // 2. Fetch roles
  const { data: rolesData, isLoading: isLoadingRoles } = useQuery(roleApi.list.query({ page: 1, limit: 100 }))
  ...
  async function handleAssign() {
    ...
    const promise = assignMutation.mutateAsync({ body: { userId: selectedUserId, locationId, roleId: selectedRoleId } })
    ...
    <Dialog open={!call.ended} onOpenChange={() => call.end()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader className="border-b pb-4">
          ...
          <DialogDescription>
            Tugaskan pengguna ke lokasi <span className="font-medium text-foreground">{locationName}</span>
          </DialogDescription>
        </DialogHeader>
                  ...
                  {users.map((u) => (
                    <button
                      ...
                      className={`w-full flex items-center gap-3 px-3 py-2 text-left hover:bg-muted/50 transition-colors ${
                        selectedUserId === u.id ? 'bg-primary/5 ring-1 ring-inset ring-primary/20' : ''
                      }`}
                    >
