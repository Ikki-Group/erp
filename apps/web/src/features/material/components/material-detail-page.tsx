...
export function MaterialDetailPage({ id }: MaterialDetailPageProps) {
  ...
  const { data: locationsResult } = useSuspenseQuery({ ...materialLocationApi.byMaterial.query({ id }) })
        ...
        action={
          <div className="flex items-center gap-2">
            ...
            <Button
              size="sm"
              onClick={() => {
                MaterialAssignToLocationDialog.call({ materialIds: [id], materialName: material.name })
              }}
            >
                    ...
                    <div className="flex flex-col items-end gap-0.5">
                      ...
                      <span
                        className={cn(
                          ...
                          recipe.isActive ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700',
                        )}
                      >
            ...
            <div className="p-2">
              {locations.length === 0 ? (
                <div className="py-8 text-center px-4">
                  <p className="text-xs text-muted-foreground mb-3">Belum di-assign ke lokasi mana pun.</p>
                  <Button
                    variant="outline"
                    size="sm"
                    className="w-full"
            ...
            <div className="p-4">
              <DataList cols={1}>
                <DataList.Item label="Terakhir Diperbarui">
                  {new Date(material.updatedAt).toLocaleString('id-ID', { dateStyle: 'long', timeStyle: 'short' })}
                </DataList.Item>
