...
function getDefaultValues(v?: MaterialSelectDto): FormDto {
  ...
  return {
    ...
    conversions: v?.conversions.map((c) => ({ uomId: c.uomId, toBaseFactor: c.toBaseFactor })) ?? [],
  }
}
  ...
  const form = useAppForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      const conversions = [
        ...
        ...value.conversions.map((c) => ({ uomId: Number(c.uomId), toBaseFactor: String(c.toBaseFactor) })),
      ]
  ...
  return (
    <form.AppForm>
      <FormConfig mode={mode} id={id} backTo={backTo}>
        <Page size="md">
          <Page.BlockHeader title={mode === 'create' ? 'Tambah Bahan Baku' : 'Edit Bahan Baku'} back={backTo} />
          <form.Form>
  ...
  return (
    <Card size="sm">
      <Card.Header className="border-b">
        ...
        <Card.Description>Satuan terkecil yang digunakan untuk mengukur bahan baku ini</Card.Description>
      </Card.Header>
      <Card.Content>
  ...
  return (
    <Card size="sm">
      <Card.Header className="border-b">
        ...
        <Card.Description>Tambahkan konversi satuan lain yang terkait dengan satuan dasar</Card.Description>
      </Card.Header>
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <div className="flex-1 max-w-[200px]">
                                  <form.AppField name={`conversions[${i}].uomId`}>
                                    {(field) => <field.Select required placeholder="Pilih satuan..." options={uoms} />}
                                  </form.AppField>
                                </div>
                                <div className="text-muted-foreground text-sm font-bold px-1">=</div>
                                <div className="flex-1 max-w-[200px]">
                                  <form.AppField name={`conversions[${i}].toBaseFactor`}>
                                    {(field) => <field.Number required placeholder="Faktor" decimalScale={10} />}
                                  </form.AppField>
                                </div>
                                <div className="flex size-8 shrink-0 items-center justify-center rounded-md border bg-muted text-xs font-semibold px-2 w-auto min-w-8">
                                  {baseUom?.label || '-'}
