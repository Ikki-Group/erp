...
const configFopts = formOptions({ validators: { onSubmit: ConfigDto }, defaultValues: {} as ConfigDto })
...
export function MaterialLocationEditSheet({ open, onOpenChange, data }: MaterialLocationEditSheetProps) {
  if (!data) return null
...
function ConfigForm({ data, onClose }: { data: MaterialLocationStockDto; onClose: () => void }) {
  ...
  const form = useAppForm({
    ...configFopts,
    defaultValues: { minStock: data.minStock, maxStock: data.maxStock, reorderPoint: data.reorderPoint },
  ...
  return (
    <form.AppForm>
      <SheetHeader className="border-b">
        ...
        <SheetDescription>
          Atur konfigurasi stok untuk <span className="font-medium text-foreground">{data.materialName}</span>
        </SheetDescription>
      </SheetHeader>
