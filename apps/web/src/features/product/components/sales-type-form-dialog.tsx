...
export const SalesTypeFormDialog = createCallable<SalesTypeFormDialogProps>((props) => {
  ...
  const selected = useQuery({ ...salesTypeApi.detail.query({ id: id! }), enabled: !!props.id, refetchOnMount: true })
  ...
  const form = useAppForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      await toast.promise(promise, toastLabelMessage(isCreate ? 'create' : 'update', 'tipe penjualan')).unwrap()
            ...
            {(field) => (
              <field.Base label="Kode Tipe" required>
                <div className="flex items-center gap-2">
                  <field.Input placeholder="Masukkan kode tipe penjualan" disabled={disabled} required />
                  <Button
                    type="button"
                    variant="outline"
                    size="icon-sm"
          ...
          <form.AppField name="name">
            {(field) => (
              <field.Base label="Nama Tipe" required>
                <field.Input placeholder="Masukkan nama tipe penjualan" disabled={disabled} required />
              </field.Base>
            )}
          </form.AppField>
          <DialogFooter>
