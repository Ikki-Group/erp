    ...
    onSubmit: async ({ value }) => {
      const payload = {
        ...
        variants: value.hasVariants
          ? value.variants.map(({ _id, id: _vId, prices, basePrice, ...v }) => ({
              ...v,
              basePrice: String(basePrice),
              prices: value.hasSalesTypePricing ? prices.map((p) => ({ ...p, price: String(p.price) })) : [],
            }))
          : [],
      }

      const promise = id ? update.mutateAsync({ body: { id, ...payload } }) : create.mutateAsync({ body: payload })
...
function PricingAndVariantsSection() {
  ...
  const { data: salesTypes } = useSuspenseQuery({ ...salesTypeApi.list.query({ page: 1, limit: 100 }) })
    ...
    >
      <div className="flex flex-col md:flex-row gap-8 p-5 border rounded-lg bg-card/50">
          ...
          {(field) => (
            <div className="flex items-start gap-4 max-w-sm">
              ...
              <div className="grid gap-1.5 leading-none">
                <Label htmlFor="toggle-variants" className="text-base font-semibold cursor-pointer">
                  Produk Memiliki Varian
                </Label>
                <p className="text-sm text-muted-foreground">
                  Aktifkan jika produk memiliki pilihan ukuran atau jenis yang berbeda (misal: Large, Hot).
                </p>
              </div>
            </div>
          )}
        ...
        <form.AppField name="hasSalesTypePricing">
          {(field) => (
            <div className="flex items-start gap-4 max-w-sm">
              <Switch id="toggle-sales-pricing" checked={field.state.value} onCheckedChange={field.handleChange} />
              <div className="grid gap-1.5 leading-none">
                <Label htmlFor="toggle-sales-pricing" className="text-base font-semibold cursor-pointer">
                  Harga Khusus Penjualan
                </Label>
                <p className="text-sm text-muted-foreground">
                  Aktifkan jika harga produk dibedakan berdasarkan platform / tipe pesanan (misal: Dine In vs GrabFood).
                </p>
              </div>
            </div>
          )}
        ...
        {!hasVariants ? (
          <div className="flex flex-col gap-6 animate-in fade-in zoom-in-95 duration-200">
            <form.AppField name="basePrice">
              {(field) => (
                <field.Base label="Harga Dasar Produk" required className="max-w-xs">
            ...
            {hasSalesTypePricing && (
              <div className="max-w-2xl">
                <Label className="mb-3 block font-semibold">Harga Tipe Penjualan</Label>
                <div className="border rounded-md overflow-hidden">
                  <Table>
                    <TableHeader className="bg-muted/50">
                      <TableRow>
                        <TableHead>Tipe Penjualan</TableHead>
                        <TableHead className="text-right w-64">Harga Khusus</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
          ...
          <div className="animate-in fade-in zoom-in-95 duration-200">
            ...
            <VariantsTable salesTypes={salesTypes.data} hasSalesTypePricing={hasSalesTypePricing} />
          </div>
        )}
      </div>
    </CardSection>
  ...
  return (
    <div className="flex items-center justify-end gap-2 group">
      <form.AppField name={`prices[${priceIndex}].price`}>
        {(field) => <field.Currency className="w-40 text-right h-8" placeholder="0" />}
      </form.AppField>
      <Button
        variant="ghost"
        size="icon-sm"
        ...
        <Table>
          <TableHeader className="bg-muted">
            <TableRow className="hover:bg-transparent">
              ...
              <TableHead className="min-w-[160px] text-right">Harga Dasar</TableHead>
              {hasSalesTypePricing &&
                salesTypes.map((st) => (
              ...
              {(arrayField) => {
                if (arrayField.state.value.length === 0) {
                  return (
                    <TableRow>
                      <TableCell
                        colSpan={hasSalesTypePricing ? 5 + salesTypes.length : 5}
                ...
                return arrayField.state.value.map((variant, i) => (
                  <TableRow key={variant._id} className={cn(variant.isDefault && 'bg-primary/5')}>
                    <TableCell className="text-center">
                      <button
                        type="button"
                        onClick={() => {
                          arrayField.state.value.forEach((_, idx) => {
                            form.setFieldValue(`variants[${idx}].isDefault`, false)
                          })
                      ...
                      >
                        <StarIcon className={cn('size-3.5', variant.isDefault && 'fill-current')} />
                      </button>
                    </TableCell>
                    <TableCell>
                    ...
                    {hasSalesTypePricing &&
                      salesTypes.map((st) => (
                        <TableCell key={st.id} className="text-right">
                          <VariantPriceCell variantIndex={i} salesTypeId={st.id} />
                        </TableCell>
                      ))}
...
function VariantPriceCell({ variantIndex, salesTypeId }: { variantIndex: number; salesTypeId: number }) {
  ...
  return (
    <div className="flex items-center justify-end gap-1 group">
      <form.AppField name={`variants[${variantIndex}].prices[${priceIndex}].price`}>
        {(field) => (
          <field.Currency
