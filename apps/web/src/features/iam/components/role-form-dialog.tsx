...
export const RoleFormDialog = createCallable<RoleFormDialogProps>((props) => {
  ...
  const selectedRole = useQuery({ ...roleApi.detail.query({ id: id! }), enabled: !!id, refetchOnMount: true })
    ...
    <form.AppForm>
      ...
      >
        <form.AppField name="name">
          {(field) => (
            <field.Input label="Nama Role" required placeholder="Contoh: Administrator Utama" disabled={disabled} />
          )}
        </form.AppField>
        ...
        <form.AppField name="description">
          {(field) => (
            <field.Textarea label="Deskripsi" placeholder="Jelaskan tanggung jawab role ini..." disabled={disabled} />
          )}
        </form.AppField>
      </FormDialog>
    </form.AppForm>
