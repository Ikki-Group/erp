...
export const UserProfilePasswordDialog = createCallable(props => {
  ...
  const changePassword = useMutation({
    mutationFn: userApi.changePassword.mutationFn,
  })
  ...
  const form = useAppForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      await toast
        .promise(promise, toastLabelMessage('update', 'password'))
        .unwrap()
      ...
      <Dialog open={!call.ended} onOpenChange={() => call.end()}>
        <DialogContent className='sm:max-w-md'>
          <DialogHeader className='border-b pb-4'>
            ...
            <DialogDescription>
              Pastikan gunakan password yang kuat dengan perpaduan huruf dan
              angka.
            </DialogDescription>
          </DialogHeader>
