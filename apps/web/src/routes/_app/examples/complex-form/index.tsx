...
type InvoiceItem = { id: string; description: string; quantity: number; unitPrice: number; tax: number }

function ComplexFormPage() {
  const [items, setItems] = useState<Array<InvoiceItem>>([
    { id: '1', description: 'Web Development Services', quantity: 1, unitPrice: 1500, tax: 0 },
  ])

  const addItem = () => {
    setItems([
      ...items,
      { id: Math.random().toString(36).substr(2, 9), description: '', quantity: 1, unitPrice: 0, tax: 0 },
    ])
  }
  ...
  return (
    <Page>
      <Page.BlockHeader title="Complex Form" description="Create a new invoice for a client." />
