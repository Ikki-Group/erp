...
function RouteComponent() {
  ...
  const form = useAppForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      const sanitizedBody = { ...value, items: value.items.map((i) => ({ materialId: i.materialId, qty: i.qty })) }
  ...
  return (
    <CardSection title="Informasi Transfer">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <form.AppField name="date">{(field) => <field.DatePicker label="Tanggal Transfer" required />}</form.AppField>
...
function TransferItemsCard() {
  ...
  return (
    <CardSection title="Bahan Baku" description="Daftar bahan baku yang akan ditransfer ke lokasi tujuan.">
      <div className="flex flex-col gap-4">
        <form.Subscribe selector={(s) => ({ items: s.values.items, sourceLocationId: s.values.sourceLocationId })}>
                        ...
                        <field.Combobox
                          ...
                          queryFn={async (search) => {
                            ...
                            const res = await materialLocationApi.stock.fetch({
                              params: { locationId: sourceLocationId, page: 1, limit: 20, search: search || undefined },
                            })
                            return res.data
                          }}
                    ...
                    <form.AppField name={`items[${i}].qty`}>
                      {(field) => (
                        <field.Number
                          ...
                          description={
                            (item as any).maxQty !== undefined ? `Maksimum: ${(item as any).maxQty}` : undefined
                          }
                        />
                      )}
                    </form.AppField>
