      ...
      <Page.Content className="flex flex-col gap-4">
        ...
        <div className="flex items-end gap-3">
          <div className="flex flex-col gap-1.5 w-full max-w-sm">
            <label className="text-sm font-medium">Lokasi Gudang (Opsional)</label>
            <DataCombobox
              value={locationId}
...
function SummaryTable({ locationId, dateFrom, dateTo }: { locationId?: number; dateFrom: string; dateTo: string }) {
  ...
  return (
    <DataTableCard title="Total Nilai Stok" table={table} isLoading={isLoading} recordCount={data?.meta.total || 0} />
  )
}
