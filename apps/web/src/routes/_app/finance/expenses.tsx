...
const mockExpenses = [
  {
    ...
    title: 'Listrik & Air Pabrik',
    category: 'Biaya Utilitas',
    amount: 3500000,
    status: 'PAID',
  },
  {
...
const columns = [
  ...
  ch.accessor('date', {
    ...
    size: 120,
    cell: ({ row }) => (
      <span className="text-sm font-medium">{toDateTimeStamp(row.original.date.toISOString()).split(',')[0]}</span>
    ),
  }),
  ch.accessor('title', {
    ...
    cell: ({ row }) => (
      <div className="flex flex-col gap-0.5">
        ...
        <span className="text-xs text-muted-foreground flex items-center gap-1">
          <TagIcon className="size-3" /> {row.original.category}
        </span>
      </div>
    ),
  }),
