...
import { ArrowRightLeftIcon, TrendingUpIcon, HashIcon } from 'lucide-react'
...
export default function ProcurementTransfersReportRoute() {
	const [dateRange, setDateRange] = useState({ dateFrom: new Date(Date.now() - 30 * 86400000), dateTo: new Date() })
	const { data, isLoading } = useQuery(procurementReportApi.transfers.query(dateRange))
	const chartData = data?.data?.data ?? []
	const summary = data?.data?.summary
	return (
		<Page size="xl">
			<Page.BlockHeader title="Transfer Stok" description="Analisis transfer stok antar lokasi." />
			<Page.Content className="flex flex-col gap-6">
				<ReportDateFilter {...dateRange} onChange={setDateRange} />
				<div className="grid gap-4 md:grid-cols-3">
					<CardStat title="Total Transfer" value={summary?.total ?? '0'} icon={ArrowRightLeftIcon} />
				...
				<ChartCard title="Transfer per Material" isLoading={isLoading}>
					<ChartContainer>
						<ResponsiveContainer width="100%" height="100%">
							<BarChart data={chartData.slice(0, 20)}>
								...
								<Bar dataKey="totalCost" fill="#10b981" name="Biaya" />
							</BarChart>
						</ResponsiveContainer>
					</ChartContainer>
				</ChartCard>
