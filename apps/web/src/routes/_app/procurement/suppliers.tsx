...
const columns = [
  ...
  ch.accessor('contact', { header: 'Kontak Utama' }),
  ch.accessor('products', {
    header: 'Total Produk Supply',
    cell: ({ row }) => <span className="font-medium text-right block pr-4">{row.original.products} Item</span>,
  }),
  ch.accessor('rating', {
    header: 'Rating Vendor',
    cell: ({ row }) => <span className="font-medium">{row.original.rating} / 5.0</span>,
  }),
  ch.accessor('status', {
    header: 'Status',
    cell: ({ row }) => {
...
function SuppliersPage() {
  ...
  return (
    <Page>
      <Page.BlockHeader title="Data Supplier" description="Direktori kelola vendor bahan baku / barang masuk Anda." />
      <Page.Content>
        <DataTableCard
