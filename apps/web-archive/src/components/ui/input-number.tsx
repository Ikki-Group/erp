 ...
 *
 * @param {number|string|null|undefined} value - The number or numeric string to format. Strings are sanitized before parsing.
 * @param {boolean} allowDecimal - Whether to allow fractional digits in the output.
 * @param {number} decimalScale - Maximum number of fraction digits to include when decimals are allowed.
 * @returns {string} The formatted number string in `id-ID` locale, or an empty string for absent or invalid input.
 */
 ...
 *
 * @param {string} value - Numeric string using dots as thousands separators and a comma as the decimal separator
 * @returns {string} The normalized numeric string with thousands separators removed and the decimal comma replaced by a dot
 */
 ...
 *
 * @param {number|null} value - The current numeric value to display; use `null` for an empty field.
 * @param {(value: number|null) => void} onChange - Callback invoked with the parsed number or `null` when the input changes.
 * @param {boolean} allowDecimal - Whether decimals are permitted (defaults to `true`).
 * @param {number} decimalScale - Maximum number of decimal places allowed when typing (defaults to `2`).
 * @param {number} max - Optional upper bound for allowed values; used only to determine whether negative input is permitted indirectly.
 * @param {number} min - Optional lower bound for allowed values; used only to determine whether negative input is permitted indirectly.
 * @param {React.Ref<HTMLInputElement>} ref - Ref forwarded to the underlying input element.
 * @returns {React.ReactElement} The rendered Input element configured for numeric entry with localized formatting and typing-aware behavior.
 */
