...
**In-memory only** (single server instance). No Redis.

- Simple Map/LRU structure in process memory.
- Survives across requests (persistent for server lifetime).
- Cleared on server restart.
- No distributed cache concerns (single instance deployment).

When scaling to multiple instances in the future: migrate to Redis or add pub/sub invalidation.
